...
function problemA () {
  /* * * * * * * * * * * * * * * * * * * * * * * * * * * *
   *
   * A. loggea el poema stanza uno y stanza dos en cualquier orden
...
//   );
  ...
  Promise.all([p1, p2])//el promise.all recibe un array y se utiliza para que mas de una promesa se ejecuten de manera simultanea
...
//   );
  ...
  let promisesTotal = filenames.map( n => {
	  return promisifiedReadFile(n)
	  .then(stanza => blue(stanza))
  })

  	Promise.all(promisesTotal)
	  .then (() => console.log("done"))
